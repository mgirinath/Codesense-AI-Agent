...
def save_review(user_id: int, language: str, code: str, static_issues: list[str], review: str) -> int:
    conn = get_connection()
    with conn, conn.cursor() as cur:
        cur.execute(
            "INSERT INTO reviews (user_id, language, code, static_issues, review, created_at) "
...
def get_history(user_id: int, limit: int = 30) -> list[dict]:
    ...
    with conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(
            "SELECT id, language, code, created_at FROM reviews "
...
def get_review_by_id(user_id: int, review_id: int) -> dict | None:
    ...
    with conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(
            "SELECT id, language, code, static_issues, review, created_at FROM reviews "
